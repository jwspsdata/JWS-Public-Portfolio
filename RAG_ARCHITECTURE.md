...
Technical manuals for field-deployed equipment often need to be usable somewhere without reliable network access — a maintenance bay with spotty connectivity, or fully air-gapped. Every model role in this pipeline (embedding, reranking, judging, generation) is architected as swappable rather than tied to one provider — none of the retrieval or generation logic assumes a hosted API. Today, the generation and judge models are hardcoded to specific Anthropic models rather than config-selected, so swapping to small, locally-hosted, open-weights models would currently require a code change, not just a config change — that's the direction the architecture is designed to support, not a capability already wired up. The one hard constraint whenever that swap happens: whatever embedded the corpus at index time must be the exact same model embedding queries at runtime, since different embedding models produce vector spaces that aren't comparable. This proof of concept runs on hosted APIs for convenience; nothing about the pipeline's logic assumes a network connection is available, but making it actually run offline today is unbuilt work, not a flag flip.
